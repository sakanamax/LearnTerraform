...
resource "google_compute_network" "project-vpc" {
  ...
  # 如果不特別設定，Terraform 會使用預設值，預設值為 false，保留預設路由。
  # delete_default_routes_on_create = false

  # 是否啟用 ULA 內部 IPv6 地址，這裡設定為 false，表示不啟用。
  # 如果不特別設定，Terraform 會使用預設值，預設值為 false，不啟用。
  # enable_ula_internal_ipv6 = false

  # 最大傳輸單元（MTU），這裡設定為 0，表示使用預設值。
  # 如果不特別設定，Terraform 會使用預設值，預設值為 1460。
  # mtu = 0

  # VPC 網路的路由模式，這裡設定為 "REGIONAL"，表示區域模式。
  # 如果不特別設定，Terraform 會使用預設值，預設值為 REGIONAL。
...
resource "google_compute_subnetwork" "subnet" {
  ...
  # 如果不特別設定，Terraform 會使用預設值，預設值為 false。
  # private_ip_google_access = false

  # 是否允許從網際網路訪問私有 IPv6 地址，這裡設定為 "DISABLE_GOOGLE_ACCESS"，表示不允許。
  # 如果不特別設定，Terraform 會使用預設值，預設值為 "DISABLE_GOOGLE_ACCESS"。
  # private_ipv6_google_access = "DISABLE_GOOGLE_ACCESS"

  # 子網路的用途，這裡設定為 "PRIVATE"，表示私有子網路。
  # 如果不特別設定，Terraform 會使用預設值，預設值為 "PRIVATE"。
  # purpose = "PRIVATE"

  # 子網路的堆疊類型，這裡設定為 "IPV4_ONLY"，表示僅支持 IPv4。
  # 如果不特別設定，Terraform 會使用預設值，預設值為 "IPV4_ONLY"。
